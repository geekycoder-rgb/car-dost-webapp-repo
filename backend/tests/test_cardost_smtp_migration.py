...
LEGACY_HOSTS = ["smtp.titan.email", "smtp.titan.mail", "smtp.titan.in", "smtp.titanemail.com"]
...
def restore_settings_at_end(admin_headers, original_settings):
    ...
    requests.put(f"{BASE_URL}/api/admin/settings", headers=admin_headers,
                 json=restore, timeout=15)


# ---------- MIGRATION tests ----------
@pytest.mark.parametrize("legacy_host", LEGACY_HOSTS[:2])  # primary 2 from review_request
def test_migration_auto_corrects_legacy_smtp_host(admin_headers, legacy_host):
    ...
    data = r2.json()
    assert data["smtp_host"] == TARGET_HOST, f"expected migrated host, got {data['smtp_host']}"
    assert data["smtp_port"] == 587, f"expected port 587, got {data['smtp_port']}"
    assert data["smtp_use_ssl"] is False, f"expected use_ssl false, got {data['smtp_use_ssl']}"


def test_migration_preserves_password(admin_headers):
    # Get current masked length
    before = requests.get(f"{BASE_URL}/api/admin/settings", headers=admin_headers, timeout=15).json()
    ...
    after = requests.get(f"{BASE_URL}/api/admin/settings", headers=admin_headers, timeout=15).json()
    ...
    assert before_masked != "", "smtp_password must be set on this env for the test to be meaningful"
...
def test_empty_smtp_password_does_not_overwrite(admin_headers):
    before = requests.get(f"{BASE_URL}/api/admin/settings", headers=admin_headers, timeout=15).json()
    before_masked = before.get("smtp_password_masked", "")
    assert before_masked != "", "Cannot test empty-overwrite safeguard if no password is currently stored"

    r = requests.put(f"{BASE_URL}/api/admin/settings", headers=admin_headers,
                     json={"smtp_password": ""}, timeout=15)
...
def test_masked_bounce_back_does_not_overwrite(admin_headers):
    ...
    r = requests.put(f"{BASE_URL}/api/admin/settings", headers=admin_headers,
                     json={"smtp_password": "••••••XYZW"}, timeout=15)
    assert r.status_code == 200
    assert r.json() == {"ok": True}
...
def test_non_secret_empty_field_is_saveable(admin_headers, original_settings):
    ...
    requests.put(f"{BASE_URL}/api/admin/settings", headers=admin_headers,
                 json={"smtp_from": restore_val}, timeout=15)
...
def test_admin_test_email_sends_ok(admin_headers):
    ...
    r = requests.post(f"{BASE_URL}/api/admin/test-email", headers=admin_headers,
                      json={"to": "customercare@cardost.in"}, timeout=30)
...
def test_non_admin_stats_403():
    ...
    requests.post(f"{BASE_URL}/api/auth/signup",
                  json={"email": email, "password": "Test@123", "name": "smoke"}, timeout=15)
    r = requests.post(f"{BASE_URL}/api/auth/login",
                      json={"email": email, "password": "Test@123"}, timeout=15)
    if r.status_code != 200:
        pytest.skip(f"could not create non-admin user: {r.status_code}")
    j = r.json()
    tok = j.get("token") or j.get("access_token")
    rr = requests.get(f"{BASE_URL}/api/admin/stats",
                      headers={"Authorization": f"Bearer {tok}"}, timeout=15)
    assert rr.status_code == 403


def test_csv_export_then_bulk_import_round_trip(admin_headers):
    exp = requests.get(f"{BASE_URL}/api/admin/products/export",
                       headers={"Authorization": admin_headers["Authorization"]}, timeout=30)
...
def test_order_status_email_and_idempotency(admin_headers):
    ...
    p1 = requests.patch(f"{BASE_URL}/api/admin/orders/{oid}/status",
                        headers=admin_headers, json={"status": target}, timeout=20)
    assert p1.status_code == 200, p1.text

    # Re-patch same status — should still 200 but not duplicate logs
    p2 = requests.patch(f"{BASE_URL}/api/admin/orders/{oid}/status",
                        headers=admin_headers, json={"status": target}, timeout=20)
